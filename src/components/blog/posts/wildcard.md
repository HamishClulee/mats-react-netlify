...
In a flash of inspiration I thought, well I could just set it up so that all subdomains are valid, then use an HTTP req to check if there should be some HTML served at this subdomain, if so, serve it, if not display a friendly 404 message and suggest that they use the app or check the subdomain.
...
- Trying to add separate certificates for the wildcard and canonical domain
- Searching the interwebs for copy and paste examples of the certbot flags that I needed, when I should have just RTFM
...
The code block below is the only nginx file for the whole site.
...
![info CORS](/images/cors.png "Example of how most people solve the pain of CORS")
...
That's it for CORS really, you could do this at the Nginx level as well, but for me this made more sense and does the job well. I can hear confused screaming coming from the security nerds "But now someone can run whatever Javascript they want on a published subdomain, and everyone could die!"

That is true, but all my endpoints are locked up with `httpOnly` cookies and JWT tokens, also all inputs are sanitised. I think we are safe, and that tinfoil hat looks a little silly on you.
